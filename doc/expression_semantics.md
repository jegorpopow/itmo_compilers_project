...
- **Type aliases**: When a type alias is declared (e.g. `type kilometers is real`), it creates a distinct type. To cast between an alias and its underlying type (or between aliases), use the **`::`** operator.
